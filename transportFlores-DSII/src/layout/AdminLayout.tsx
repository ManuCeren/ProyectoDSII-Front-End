...
export default function AdminLayout() {
  const [view, setView] = useState<
    'dashboard' | 'clientes' | 'unidades' | 'envios' | 'usuarios' | 'conductores' | 'mantenimiento' | 'facturacion'| 'rutas' | 'proyecciones' 
  >('dashboard');
    ...
    <div className="d-flex m-0 p-0" style={{ minHeight: '100vh' }}>
      ...
      <aside className={`sidebar bg-dark text-white p-3 ${sidebarCollapsed ? 'collapsed' : ''}`}>
        ...
        <nav className="nav flex-column">
          ...
          <button className="nav-link text-white d-flex align-items-center" onClick={() => handleViewChange('rutas')}>
           ...
           <button className="nav-link text-white d-flex align-items-center" onClick={() => handleViewChange('proyecciones')}>
            <i className="bi-graph-up"></i>
            {!sidebarCollapsed && <span>Proyeccion</span>}
          </button>
      ...
      <div className="d-flex flex-column flex-grow-1" style={{ minHeight: '100vh' }}>
        ...
        <main className="p-4 flex-grow-1 overflow-auto">
          ...
          {view === 'proyecciones' && <Proyecciones handleViewChange={handleViewChange} />}
        </main>
      </div>
    </div>
  );
