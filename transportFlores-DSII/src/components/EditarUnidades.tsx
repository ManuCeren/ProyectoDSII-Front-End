...
export function EditarUnidades(){
    const {id} = useParams<{id:string}>()
    const [unidad,setUnidad] = useState<IUnidades>(initialUnidades);
    const navigate = useNavigate();
    useEffect(()=>{
        const obtenerUnidades = async()=>{
            const response = await fetch(`${appsettings.apiUrl}Unidades/Obtener/${id}`)
            if(response.ok){
    ...
    const guardar= async()=>{
        const response = await fetch(`${appsettings.apiUrl}Unidades/Editar`,{
            method: 'PUT',
            headers:{
